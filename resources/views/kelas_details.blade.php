...
{{-- Course Banner --}}
<div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4" style="background: linear-gradient(135deg, {{ $course->color }}f0, {{ $course->color }}aa), url('https://images.unsplash.com/photo-1516321318423-f06f85e504b3?q=80&w=1200&auto=format&fit=crop') center/cover; color: #fff;">
    <div class="px-4 py-5 d-flex flex-column justify-content-between position-relative" style="min-height: 180px; z-index: 2; background: rgba(0,0,0,0.15);">
        <div class="d-flex align-items-start mb-3 justify-content-between w-100 flex-wrap gap-2">
            <div>
                <span class="badge rounded-pill bg-white text-dark mb-2 px-3 py-2 fw-semibold" style="font-size: 0.72rem; color: {{ $course->color }} !important; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
                    Kode Kelas: <strong>{{ $course->code }}</strong>
                </span>
                <h2 class="fw-bold mb-1" style="font-size: 1.8rem; letter-spacing: -0.02em;">{{ $course->name }}</h2>
                <p class="mb-0 text-white-50 small fw-medium">{{ $course->subject }}</p>
            </div>
            @if($userRole === 'student')
                <form action="{{ route('courses.leave', $course->id) }}" method="POST" onsubmit="return confirm('Apakah kamu yakin ingin keluar dari kelas {{ $course->name }}?')">
                    @csrf
...
<div class="tab-content" id="courseTabContent">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-3 d-none d-lg-block">
                <div class="card border-0 shadow-sm rounded-4 bg-white p-3 mb-3">
                    ...
                    <div class="text-secondary small mb-3">
                        Dibuat oleh <strong>{{ $course->teacher_name }}</strong>. Gunakan kode kelas untuk mengajak siswa bergabung.
                    </div>
                    <div class="p-2.5 rounded-3 bg-light d-flex align-items-center justify-content-between">
                        <span class="font-monospace text-dark fw-bold text-uppercase" style="letter-spacing: 0.05em;">{{ $course->code }}</span>
                        <button class="btn btn-sm text-primary p-1 border-0 bg-transparent" onclick="copyCode('{{ $course->code }}')" title="Salin kode">
                ...
                <div class="card border-0 shadow-sm rounded-4 bg-white p-3">
                    ...
                    @if($upcomingTasks->count() > 0)
                        <div class="d-flex flex-column gap-2">
                            @foreach($upcomingTasks as $utask)
                            <div class="p-2 rounded-3" style="background-color: #ceeaf040; border-left: 3px solid {{ $course->color }};">
                                <div class="fw-semibold text-dark text-truncate" style="font-size: 0.78rem;">{{ $utask->title }}</div>
                                <div class="text-secondary" style="font-size: 0.7rem;">Batas: {{ $utask->due_date }}</div>
                            </div>
                            @endforeach
                        </div>
                    @else
            ...
            <div class="col-lg-9 col-12">
                ...
                <div class="d-flex flex-column gap-3">
                    {{-- Loop through materials and tasks as a stream --}}
                        ...
                        <div class="card border-0 shadow-sm rounded-4 bg-white p-4">
                            <div class="d-flex align-items-start gap-3 mb-3">
                                <div class="rounded-circle d-flex align-items-center justify-content-center flex-shrink-0"
                                     style="width: 40px; height: 40px; background-color: {{ $item['bg'] }};">
                                    <i class="bi {{ $item['icon'] }}" style="color: {{ $item['color'] }}; font-size: 1.1rem;"></i>
                                </div>
                                <div class="flex-grow-1">
                                    <div class="fw-bold text-dark small" style="font-size: 0.9rem;">{{ $item['title'] }}</div>
                                    <div class="text-secondary" style="font-size: 0.75rem;">{{ $item['date']->diffForHumans() }}</div>
                                </div>
                            </div>
                            <h6 class="fw-bold text-dark mb-1" style="font-size: 0.95rem;">{{ $item['content'] }}</h6>
                            @if($item['desc'])
                                <p class="text-secondary small mb-3 text-break">{{ $item['desc'] }}</p>
                            @endif
                            <div class="d-flex justify-content-start border-top pt-2 mt-2">
                                @if($item['type'] === 'materi')
                                    <a href="{{ route('materials.show', [$course->id, $item['id']]) }}"
                                       class="btn btn-sm px-3 rounded-2 fw-semibold text-decoration-none"
                                       style="color: {{ $course->color }}; background-color: {{ $course->color }}15; font-size: 0.78rem; border: none;">
                                        Buka Materi <i class="bi bi-arrow-right ms-1"></i>
                                    </a>
                                @else
                                    <a href="{{ route('tasks.show', [$course->id, $item['id']]) }}"
                                       class="btn btn-sm px-3 rounded-2 fw-semibold text-decoration-none"
                                       style="color: {{ $item['color'] }}; background-color: {{ $item['color'] }}15; font-size: 0.78rem; border: none;">
                        ...
                        <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-sm-nowrap">
                            <div class="d-flex align-items-center gap-3">
                                <div class="rounded-3 d-flex align-items-center justify-content-center flex-shrink-0"
                                     style="width: 42px; height: 42px; background-color: {{ $course->color }}10;">
                                    <i class="bi bi-file-earmark-text-fill" style="color: {{ $course->color }}; font-size: 1.25rem;"></i>
                                </div>
                                <div>
                                    <h6 class="fw-bold text-dark mb-0.5" style="font-size: 0.92rem;">{{ $mat->title }}</h6>
                                    <span class="text-secondary" style="font-size: 0.75rem;">Diunggah {{ $mat->created_at->isoFormat('D MMM YYYY') }}</span>
                                </div>
                            </div>
                            <a href="{{ route('materials.show', [$course->id, $mat->id]) }}" class="btn btn-sm px-3 rounded-3 fw-semibold flex-shrink-0" 
                               style="font-size: 0.8rem; background-color: {{ $course->color }}15; color: {{ $course->color }}; border: none; padding: 0.45rem 1rem;">
                                Buka Materi <i class="bi bi-arrow-right ms-1"></i>
                            </a>
                        </div>
                    </div>
                ...
                <div class="px-4 py-3 d-flex align-items-center justify-content-between gap-3 {{ $i !== $tasks->count() - 1 ? 'border-bottom' : '' }} flex-wrap flex-sm-nowrap">
                    <div class="d-flex align-items-center gap-3">
                        <div class="rounded-3 d-flex align-items-center justify-content-center flex-shrink-0"
                            ...
                            <i class="bi bi-clipboard-check" style="color: {{ $course->color }}; font-size: 1.1rem;"></i>
                        </div>
                        <div>
                    ...
                    <div class="d-flex align-items-center gap-2 ms-auto ms-sm-0 flex-shrink-0">
                        ...
                        @endif
                        <a href="{{ route('tasks.show', [$course->id, $task->id]) }}" class="btn btn-sm text-white px-3 py-1.5 rounded-3 fw-bold" 
                           style="font-size: 0.78rem; background-color: {{ $course->color }}; border: none;">
                            Buka Tugas
                        </a>
                    </div>
                </div>
    ...
    <div class="tab-pane fade" id="anggota" role="tabpanel" aria-labelledby="anggota-tab">
        <div class="row g-4">
            ...
            <div class="col-12">
                <div class="card border-0 shadow-sm rounded-4 bg-white p-4">
                    <h5 class="fw-bold text-dark border-bottom pb-3 mb-3" style="font-size: 1.1rem; border-color: #e2e8f0 !important; color: {{ $course->color }} !important;">
                        Pengajar
                    </h5>
                    <div class="d-flex flex-column gap-3">
                        ...
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-secondary" style="font-size: 0.75rem;">{{ $teacher->email }}</div>
                            </div>
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="col-12">
                <div class="card border-0 shadow-sm rounded-4 bg-white p-4">
                    <div class="d-flex align-items-center justify-content-between border-bottom pb-3 mb-3" style="border-color: #e2e8f0 !important;">
                        <h5 class="fw-bold text-dark mb-0" style="font-size: 1.1rem; color: {{ $course->color }} !important;">
                            Teman Sekelas
                        </h5>
                        <span class="badge rounded-pill fw-semibold" style="background-color: {{ $course->color }}1a; color: {{ $course->color }}; font-size: 0.78rem;">
                            ...
                            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap">
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-secondary" style="font-size: 0.72rem;">{{ $student->email }}</div>
                                    </div>
                                </div>
                                @if($userRole === 'teacher')
                                    <form action="{{ route('courses.kick', [$course->id, $student->id]) }}" method="POST" onsubmit="return confirm('Keluarkan siswa {{ $student->name }} dari kelas ini?')">
                                        @csrf
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 16px; overflow: hidden;">
            <div class="modal-header border-0 pb-0 px-4 pt-4">
                <div class="d-flex align-items-center gap-3">
                    <div class="d-flex align-items-center justify-content-center rounded-3"
                         style="width: 44px; height: 44px; background-color: {{ $course->color }}1a; flex-shrink: 0;">
                        <i class="bi bi-file-earmark-plus-fill" style="color: {{ $course->color }}; font-size: 1.25rem;"></i>
                    </div>
                    <div>
            ...
            <form action="{{ route('materials.store', $course->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-footer border-0 px-4 pb-4 pt-2 gap-2">
                    ...
                    <button type="submit" class="btn text-white flex-grow-1" style="background-color: {{ $course->color }}; border: none; font-weight: 600; font-size: 0.85rem; padding: 0.55rem 1rem; border-radius: 6px;">
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 16px; overflow: hidden;">
            <div class="modal-header border-0 pb-0 px-4 pt-4">
                <div class="d-flex align-items-center gap-3">
                    <div class="d-flex align-items-center justify-content-center rounded-3"
                         style="width: 44px; height: 44px; background-color: {{ $course->color }}1a; flex-shrink: 0;">
                        <i class="bi bi-clipboard-plus-fill" style="color: {{ $course->color }}; font-size: 1.25rem;"></i>
                    </div>
                    <div>
            ...
            <form action="{{ route('tasks.store', $course->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body px-4 pt-3 pb-2">
                    <div class="mb-3">
