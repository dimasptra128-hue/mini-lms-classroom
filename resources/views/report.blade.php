    ...
    <div class="col-6 col-md-3">
        <div class="card report-summary-card border-0 shadow-sm rounded-4 h-100 overflow-hidden interactive-card" style="position: relative;">
            ...
            <div class="card-body p-3 pt-4">
                ...
                @if($overallAvg)
                    <div class="mt-1">
                        <span class="badge rounded-pill px-2" style="background: {{ $overallGradeColor }}20; color: {{ $overallGradeColor }}; font-size: 0.75rem;">
    ...
    <div class="col-6 col-md-3">
        <div class="card report-summary-card border-0 shadow-sm rounded-4 h-100 overflow-hidden interactive-card" style="position: relative;">
            ...
            <div class="card-body p-3 pt-4">
                ...
                <div class="fw-bold" style="font-size: 2rem; color: #1e293b; line-height: 1.1;">{{ $totalTasks }}</div>
    ...
    <div class="col-6 col-md-3">
        <div class="card report-summary-card border-0 shadow-sm rounded-4 h-100 overflow-hidden interactive-card" style="position: relative;">
            ...
            <div class="card-body p-3 pt-4">
                ...
                <div class="fw-bold" style="font-size: 2rem; color: #1e293b; line-height: 1.1;">{{ $totalSelesai }}</div>
                <div class="text-secondary mt-1" style="font-size: 0.75rem;">
                    {{ $totalTasks > 0 ? round(($totalSelesai / $totalTasks) * 100) : 0 }}% dari total
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-6 col-md-3">
        <div class="card report-summary-card border-0 shadow-sm rounded-4 h-100 overflow-hidden interactive-card" style="position: relative;">
            ...
            <div class="card-body p-3 pt-4">
                ...
                <div class="fw-bold" style="font-size: 2rem; color: #1e293b; line-height: 1.1;">{{ $totalGraded }}</div>
...
@forelse($reportData as $data)
    <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
        ...
        <div class="card-header border-0 p-4 pb-3 card-header-toggle"
             style="background: {{ $data->course->color }}12; cursor: pointer;"
             data-bs-toggle="collapse"
             data-bs-target="#collapseCourse{{ $data->course->id }}"
             aria-expanded="true">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div class="d-flex align-items-center gap-3">
                    <div>
                        <h5 class="fw-bold mb-0 text-dark" style="font-size: 1rem;">{{ $data->course->name }}</h5>
                        <small class="text-secondary">{{ $data->course->subject }} · {{ $data->course->teacher_name }}</small>
                    </div>
                </div>
                
                {{-- Course avg badge & Chevron toggle --}}
                <div class="d-flex align-items-center gap-3">
                    <div class="text-end">
                        @if($data->avgScore !== null)
                            <div class="fw-bold" style="font-size: 1.75rem; color: {{ $data->avgScoreColor }}; line-height: 1;">
                                {{ $data->avgScore }}
                            </div>
                            <small style="color: {{ $data->avgScoreColor }}; font-weight: 600;">Predikat {{ $data->avgScoreGrade }}</small>
                        @else
            ...
            <div class="d-flex gap-4 mt-3 flex-wrap">
                <div class="text-center">
                    <div class="fw-bold" style="font-size: 0.9rem; color: {{ $data->course->color }};">{{ $data->allTasks }}</div>
                    <div class="text-secondary" style="font-size: 0.72rem;">Total Tugas</div>
                </div>
                <div class="text-center">
                    <div class="fw-bold" style="font-size: 0.9rem; color: #16a34a;">{{ $data->selesai }}</div>
                    <div class="text-secondary" style="font-size: 0.72rem;">Selesai</div>
                </div>
                @if($data->maxScore !== null)
                <div class="text-center">
                    <div class="fw-bold" style="font-size: 0.9rem; color: #b45309;">{{ $data->maxScore }}</div>
                    <div class="text-secondary" style="font-size: 0.72rem;">Tertinggi</div>
                </div>
                <div class="text-center">
                    <div class="fw-bold" style="font-size: 0.9rem; color: #dc2626;">{{ $data->minScore }}</div>
            ...
            <div class="mt-3">
                <div class="d-flex justify-content-between mb-1">
                    ...
                    <small class="fw-semibold" style="font-size: 0.72rem; color: {{ $data->course->color }};">{{ $data->progress }}%</small>
                </div>
                <div class="rounded-pill overflow-hidden" style="height: 6px; background: {{ $data->course->color }}25;">
                    <div class="rounded-pill" style="height: 100%; width: {{ $data->progress }}%; background: {{ $data->course->color }}; transition: width 0.8s ease;"></div>
                </div>
            </div>
        </div>

        {{-- Collapse Wrapper --}}
        <div class="collapse show" id="collapseCourse{{ $data->course->id }}">
            {{-- Task Table --}}
                                    ...
                                    <tr class="task-row">
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <div class="fw-semibold text-dark" style="font-size: 0.85rem;">{{ $task->title }}</div>
                                            </div>
                                            @if($task->description)
                                        ...
                                        <td class="px-3 py-3">
                                            <span class="text-secondary" style="font-size: 0.8rem;">
                                                <i class="bi bi-calendar2 me-1"></i>{{ $task->due_date ?? '-' }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="px-3 py-3 text-center">
                                            <span class="badge rounded-pill px-2.5 py-1.5 d-inline-flex align-items-center gap-1 cursor-pointer"
                                                  style="background: {{ $task->status_bg }}; color: {{ $task->status_color }}; font-size: 0.72rem; font-weight: 600; transition: transform 0.2s;"
                                                  onmouseover="this.style.transform='scale(1.05)'"
                                                  onmouseout="this.style.transform='scale(1)'">
                                                <i class="bi {{ $task->status_icon }}"></i>{{ $task->status }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="px-3 py-3 text-center">
                                            @if($task->score !== null)
                                                <div class="fw-bold" style="font-size: 1.15rem; color: {{ $task->score_color }};">
                                                    {{ $task->score }}
                                                </div>
                                            @else
                                        ...
                                        <td class="px-3 py-3 text-center">
                                            @if($task->score !== null)
                                                <span class="fw-bold d-inline-flex align-items-center justify-content-center rounded-circle grade-badge"
                                                      style="width: 30px; height: 30px; background: {{ $task->score_color }}18; color: {{ $task->score_color }}; font-size: 0.8rem; cursor: help;"
                                                      title="Nilai {{ $task->score }}">
                                                    {{ isset($task->_model) ? $task->_model->scoreGrade() : (
                                        ...
                                        <td class="px-4 py-3">
                                            @if($task->feedback)
                                                <div class="d-flex align-items-start gap-2 tooltip-container" data-tooltip="{{ $task->feedback }}">
                                                    <i class="bi bi-chat-left-quote-fill mt-1" style="font-size: 0.82rem; cursor: pointer; color: {{ $data->course->color }};"></i>
                                                    <span class="text-secondary text-truncate" style="font-size: 0.78rem; font-style: italic; max-width: 200px; display: inline-block;">
                                                        {{ Str::limit($task->feedback, 65) }}
                                                    </span>
                                                </div>
                                            @else
...
@empty
    <div class="card border-0 shadow-sm rounded-4 p-5 text-center">
        ...
        <a href="{{ route('courses') }}" class="btn btn-gabung-kelas mx-auto" style="max-width: 180px;">
...
<div class="card border-0 rounded-4 mt-2 mb-4" style="background: #f8fafc;">
    <div class="card-body py-3 px-4">
        <div class="d-flex align-items-center flex-wrap gap-3">
            ...
            @foreach([['A','≥ 90','#16a34a','Sangat Baik'],['B','80–89','#1F7A8C','Baik'],['C','70–79','#b45309','Cukup'],['D','60–69','#dc2626','Kurang / Perlu Perbaikan'],['E','< 60','#7f1d1d','Sangat Kurang']] as [$g,$r,$c,$d])
            <div class="d-flex align-items-center gap-1 tooltip-container" data-tooltip="{{ $d }}" style="cursor: help;">
                <span class="fw-bold d-inline-flex align-items-center justify-content-center rounded-circle grade-badge"
                      style="width: 22px; height: 22px; background: {{ $c }}18; color: {{ $c }}; font-size: 0.7rem;">{{ $g }}</span>
                <small style="color: #64748b; font-size: 0.75rem;">{{ $r }}</small>
            </div>
            @endforeach
        </div>
    </div>
