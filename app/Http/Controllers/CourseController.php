...
class CourseController extends Controller
{
    ...
    {
        // 1. Ambil data kelas dari MySQL beserta seluruh relasinya
        ...
        // 2. Tentukan Role Pengguna yang sedang login (Teacher atau Student)
        $userRole = ($course->creator_id === auth()->id()) ? 'teacher' : 'student';

        // 3. Tentukan pengajar dan siswa berdasarkan pivot `role`
        $teachers = $course->users()->wherePivot('role', 'teacher')->get();
        $students = $course->users()->wherePivot('role', 'student')->get();

        // Fallback: jika tidak ada pengajar pada pivot, coba ambil user creator_id
        if ($teachers->isEmpty()) {
            $creator = \App\Models\User::find($course->creator_id);
            if ($creator) {
                $teachers = collect([$creator]);
            } else {
                // Jika tidak ditemukan user di DB, gunakan nama fallback dari course
                $teachers = collect([(object)['name' => $course->teacher_name, 'email' => '']]);
            }
        }

        // 4. Ambil materi dan tugas langsung dari DB agar tidak bergantung pada atribut JSON yang menimpa relasi
        $materialModels = Material::where('course_id', $course->id)
            ->orderBy('created_at', 'desc')
            ->get();

        $taskModels = Task::where('course_id', $course->id)
            ->orderBy('due_date', 'asc')
            ->get();

        // Siapkan tugas mendatang (max 3)
        $upcomingTasks = $taskModels->take(3);

        // 5. Gabungkan Materi & Tugas ke dalam satu Koleksi untuk "Alur Kelas (Stream Feed)"
        $feedItems = collect();

        // PROTEKSI: Pastikan $course->materials tidak null dan bisa di-foreach
        if (!empty($course->materials) && (is_array($course->materials) || is_object($course->materials))) {
            foreach ($course->materials as $material) {
                $feedItems->push([
                    'id' => $material->id,
                    'type' => 'materi',
                    'title' => 'Mengirimkan Materi Baru',
        ...
        // PROTEKSI: Pastikan $course->tasks tidak null dan bisa di-foreach
        if ($taskModels->count() > 0) {
            foreach ($taskModels as $task) {
                $feedItems->push([
                    ...
                    'content' => $task->title,
                    'desc' => \Illuminate\Support\Str::limit($task->description, 100),
        ...
        // Urutkan feed berdasarkan tanggal terbaru
        $feedItems = $feedItems->sortByDesc('date')->values();

        // Urutkan feed berdasarkan tanggal terbaru
        $feedItems = $feedItems->sortByDesc('date')->values();

        // 6. Return ke view 'kelas_details'
        return view('kelas_details', compact(
            'course', 
            'userRole', 
            'teachers', 
            'students', 
